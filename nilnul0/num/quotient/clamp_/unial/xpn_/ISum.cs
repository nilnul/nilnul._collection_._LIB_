...
namespace nilnul.num.quotient.clamp_.unial.xpn_
{
	...
	internal class ISum
	{
		...
		/*
	集合里元素个数为可数无穷（可以和自然数集建立一一对应关系）时，可以把集合里的数一个一个加起来，再取极限作为所有元素的和，但前提是这个极限和求和顺序无关。有的时候这个极限和求和顺序是无关的（比如这个集合里所有元素的绝对值以某一顺序相加，结果并不发散），有的时候这个极限和求和顺序是有关的（比如【-1，1】上所有有理数组成的集合）。	 
		 */
	}
}
